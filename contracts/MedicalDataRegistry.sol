...
contract MedicalDataRegistry {
    ...
    event DataRegistered(bytes32 indexed dataHash, address indexed owner, string ipfsHash);
    event DataPurchased(bytes32 indexed dataHash, address indexed buyer, uint256 price);
    event PatientRegistered(address indexed patient, string name);
    event DataMadePublic(bytes32 indexed dataHash, uint256 price);
    
    modifier onlyOwner(bytes32 dataHash) {
        require(medicalDataRegistry[dataHash].owner == msg.sender, "Not the data owner");
        _;
    }
    ...
    function registerMedicalData(
        ...
        bytes32 dataHash = keccak256(abi.encodePacked(_ipfsHash, msg.sender, block.timestamp));
        
        require(!medicalDataRegistry[dataHash].exists, "Data already registered");
    ...
    function makeDataPublic(bytes32 _dataHash, uint256 _price) public 
        onlyOwner(_dataHash) 
    ...
    function verifyDataIntegrity(bytes32 _dataHash) public view 
        dataExists(_dataHash) 
    ...
    function getPatientData(address _patient) public view returns (bytes32[] memory) {
        return patientDataHashes[_patient];
    }
    
    function hasAccessToData(bytes32 _dataHash, address _user) public view returns (bool) {
    ...
    function getMedicalData(bytes32 _dataHash) public view 
        dataExists(_dataHash) 
        returns (
